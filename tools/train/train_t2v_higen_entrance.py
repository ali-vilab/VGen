...
def train_t2v_higen_entrance(cfg_update,  **kwargs):
...
def worker(gpu, cfg):
    ...
    for step in range(resume_step, cfg.num_steps + 1): 
        ...
        un_video_data = (video_data * torch.Tensor(cfg.std).to(gpu)[None, :, None, None] + torch.Tensor(cfg.mean).to(gpu)[None, :, None, None])
        un_video_data = un_video_data[:, :, :, (cfg.resolution[0]-cfg.resolution[1]) // 2: (cfg.resolution[0]-cfg.resolution[1]) // 2 + cfg.resolution[1]]
        vit_video_data = (torch.nn.functional.interpolate(un_video_data, size=cfg.embedder.vit_resolution) - torch.Tensor(cfg.vit_mean).to(gpu)[None, :, None, None]) / torch.Tensor(cfg.vit_std).to(gpu)[None, :, None, None]
        ...
        with torch.no_grad():
            video_visual_img, _ = clip_encoder(image=vit_video_data, text=None)
            video_visual_img = video_visual_img.view(batch_size, frames_num, -1)
            video_visual_img = torch.nn.functional.normalize(video_visual_img, dim=-1, p=2)
            video_img_sim = torch.bmm(video_visual_img, video_visual_img.transpose(1, 2)) * 5.0 - 4.0

        # forward
        model_kwargs = {'y': y_words}
        if video_data.size(2) > 1:
            model_kwargs['spat_prior'] = video_data[:, :, frames_num//2, :, :].detach()
            model_kwargs['motion_cond'] = ((video_data[:, :, 1:, ...] - video_data[:, :, :-1, ...]).abs().norm(dim=(3, 4)).norm(dim=1) * 10).long()
            model_kwargs['appearance_cond'] = video_img_sim
        else:
            model_kwargs['spat_prior'] = torch.zeros_like(video_data).squeeze(2)
            model_kwargs['motion_cond'] = torch.tensor([0] * y_words.size(0),dtype=torch.long, device=gpu)
            model_kwargs['appearance_cond'] = video_img_sim.repeat(1, 1, max(cfg.frame_lens))
            ...
            with torch.no_grad():
                try:
                    visual_kwards = [
                        {
                            'y': y_words_0[:viz_num],
                            'spat_prior': model_kwargs['spat_prior'][:viz_num],
                            'motion_cond': model_kwargs['motion_cond'][:viz_num],
                            'appearance_cond': model_kwargs['appearance_cond'][:viz_num],
                        },
                        {
                            'y': zero_y_negative.repeat(y_words_0.size(0), 1, 1)[:viz_num],
                            'spat_prior': model_kwargs['spat_prior'][:viz_num],
                            'motion_cond': model_kwargs['motion_cond'][:viz_num],
                            'appearance_cond': model_kwargs['appearance_cond'][:viz_num],
                        }
