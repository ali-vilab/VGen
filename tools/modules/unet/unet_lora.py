...
# ------------------------------------------------------------------------
# ----------------------------- Notice -----------------------------------
# We provide an instantiation of LoRA tuning for ModelScopeT2V.
...
# ### Original MemoryEfficientCrossAttention, without LoRA
...
class LoRA(nn.Module):
    def __init__(self, in_dim, out_dim, rank, network_alpha=None):
        super().__init__()
        self.down_linear = nn.Linear(in_dim, rank, bias=False)
        self.up_linear = nn.Linear(rank, out_dim, bias=False)
        self.network_alpha = network_alpha

        nn.init.normal_(self.down_linear.weight, std=1 / rank)
    ...
    def forward(self, x):
        ...
        if self.network_alpha is not None:
            # print('self.network_alpha is not None: ', self.network_alpha)
            x *= self.network_alpha / self.rank
        return x
...
class MemoryEfficientCrossAttention(nn.Module):
    # https://github.com/MatthieuTPHR/diffusers/blob/d80b531ff8060ec1ea982b65a1b8df70f73aa67c/src/diffusers/models/attention.py#L223
    def __init__(self, query_dim, context_dim=None, heads=8, dim_head=64, dropout=0.0, 
                       use_lora=False, lora_rank=None, lora_alpha=None):
        super().__init__()
        print(f"Setting up {self.__class__.__name__}. Query dim is {query_dim}, context_dim is {context_dim} and using "
              f"{heads} heads.")
        inner_dim = dim_head * heads
        ...
        if use_lora:
            self.to_q_lora = LoRA(query_dim, inner_dim, lora_rank, lora_alpha)
            self.to_k_lora = LoRA(context_dim, inner_dim, lora_rank, lora_alpha)
            self.to_v_lora = LoRA(context_dim, inner_dim, lora_rank, lora_alpha)
            self.to_out_lora = LoRA(inner_dim, query_dim, lora_rank, lora_alpha)
    ...
    def forward(self, x, context=None, mask=None):
        ...
        q, k, v = map(
            ...
            .permute(0, 2, 1, 3)
            .reshape(b * self.heads, t.shape[1], self.dim_head)
            .contiguous(),  # might need to be deleted
            (q, k, v),
        )
        ...
        if self.use_lora:
            return self.to_out(out) + self.to_out_lora(out)
        else:
            return self.to_out(out)
...
class BasicTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, n_heads, d_head, dropout=0., context_dim=None, gated_ff=True, checkpoint=True,
        ...
        self.attn1 = attn_cls(query_dim=dim, heads=n_heads, dim_head=d_head, dropout=dropout,
                              ...
                              use_lora=use_lora, lora_rank=lora_rank, lora_alpha=lora_alpha)  # is a self-attention if not self.disable_self_attn
        self.ff = FeedForward(dim, dropout=dropout, glu=gated_ff)
        self.attn2 = attn_cls(query_dim=dim, context_dim=context_dim,
                              heads=n_heads, dim_head=d_head, dropout=dropout,
                              use_lora=use_lora, lora_rank=lora_rank, lora_alpha=lora_alpha)  # is self-attn if context is none
...
class UNetSD_LoRA(nn.Module):
    def __init__(self,
        ...
        super(UNetSD_LoRA, self).__init__()
        ...
        self.use_fps_condition = use_fps_condition # False
