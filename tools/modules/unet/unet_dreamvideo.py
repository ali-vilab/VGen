...
class UNetSD_DreamVideo(nn.Module):
    def __init__(self,
            config=None,
            in_dim=4,
            dim=512,
            ...
            zero_y = None,
            spatial_adapter_list=[],
            spatial_adapter_position_list=['', 'parallel', ''],
            spatial_adapter_hidden_dim=None,
            temporal_adapter_list=[],
            temporal_adapter_position_list=['parallel', 'parallel', 'parallel'],
            temporal_adapter_condition_dim=None,
            temporal_adapter_hidden_dim=None,
            **kwargs):
        super(UNetSD_DreamVideo, self).__init__()
        ...
        if temporal_attention:
            if USE_TEMPORAL_TRANSFORMER:
                init_block.append(TemporalTransformerWithAdapter(dim, num_heads, head_dim, depth=transformer_depth, context_dim=context_dim,
                                disable_self_attn=disabled_sa, use_linear=use_linear_in_temporal, multiply_zero=use_image_dataset,
                                adapter_list=temporal_adapter_list, adapter_position_list=temporal_adapter_position_list,
                                adapter_hidden_dim=temporal_adapter_hidden_dim, adapter_condition_dim=temporal_adapter_condition_dim))
            else:
                ...
                if scale in attn_scales:
                    block.append(
                            SpatialTransformerWithAdapter(
                                ...
                                adapter_list=spatial_adapter_list, adapter_position_list=spatial_adapter_position_list,
                                adapter_hidden_dim=spatial_adapter_hidden_dim
                            )
                    )
                    if self.temporal_attention:
                        if USE_TEMPORAL_TRANSFORMER:
                            block.append(TemporalTransformerWithAdapter(out_dim, out_dim // head_dim, head_dim, depth=transformer_depth, context_dim=context_dim,
                                disable_self_attn=disabled_sa, use_linear=use_linear_in_temporal, multiply_zero=use_image_dataset,
                                adapter_list=temporal_adapter_list, adapter_position_list=temporal_adapter_position_list,
                                adapter_hidden_dim=temporal_adapter_hidden_dim, adapter_condition_dim=temporal_adapter_condition_dim))
                        else:
            ...
            if USE_TEMPORAL_TRANSFORMER:
                self.middle_block.append(
                 TemporalTransformerWithAdapter( 
                            ...
                            multiply_zero=use_image_dataset, adapter_list=temporal_adapter_list, adapter_position_list=temporal_adapter_position_list,
                            adapter_hidden_dim=temporal_adapter_hidden_dim, adapter_condition_dim=temporal_adapter_condition_dim
                        )
                )
            else:
    ...
    def forward(self, 
        ...
        **kwargs):
        ...
        if context_image is not None and context_image.shape[1] == 1:
            context_image = context_image.repeat_interleave(repeats=f,dim=1)
        
        x = rearrange(x, 'b c f h w -> (b f) c h w')
        xs = []
        for block in self.input_blocks:
            x = self._forward_single(block, x, embeddings, context, time_rel_pos_bias, focus_present_mask, video_mask, adapter_condition=context_image, adapter_condition_lam=ag_strength)
            xs.append(x)
        ...
        for block in self.output_blocks:
            ...
            x = self._forward_single(block, x, embeddings, context, time_rel_pos_bias,focus_present_mask, video_mask, reference=xs[-1] if len(xs) > 0 else None, adapter_condition=context_image, adapter_condition_lam=ag_strength)
    ...
    def _forward_single(self, module, x, e, context, time_rel_pos_bias, focus_present_mask, video_mask, reference=None, adapter_condition=None, adapter_condition_lam=1):
        if isinstance(module, ResidualBlock):
            ...
            module = checkpoint_wrapper(module) if self.use_checkpoint else module
            x = module(x, context)
        elif isinstance(module, SpatialTransformerWithAdapter):
            module = checkpoint_wrapper(module) if self.use_checkpoint else module
            x = module(x, context)
            ...
            x = module(x, context)
            x = rearrange(x, 'b c f h w -> (b f) c h w')
        elif isinstance(module, TemporalTransformerWithAdapter):
            module = checkpoint_wrapper(module) if self.use_checkpoint else module
            x = rearrange(x, '(b f) c h w -> b c f h w', b = self.batch)
            x = module(x, context, adapter_condition, adapter_condition_lam)
            x = rearrange(x, 'b c f h w -> (b f) c h w')
            ...
            module = checkpoint_wrapper(module) if self.use_checkpoint else module
            x = module(x, context)
        elif isinstance(module, BasicTransformerBlockWithAdapter):
            module = checkpoint_wrapper(module) if self.use_checkpoint else module
            x = module(x, context, adapter_condition, adapter_condition_lam)
            ...
            for block in module:
                x = self._forward_single(block,  x, e, context, time_rel_pos_bias, focus_present_mask, video_mask, reference, adapter_condition=adapter_condition, adapter_condition_lam=adapter_condition_lam)
        else:
            x = module(x)
        return x
