...
class ImageCustomDataset(Dataset):
    def __init__(self, 
        ...
        self.use_mask_diffusion = use_mask_diffusion
    ...
    def __getitem__(self, index):
        ...
        try:
            ref_frame, vit_frame, video_data, mask, caption = self._get_image_data(data_dir, file_path)
        ...
        return ref_frame, vit_frame, video_data, mask, caption, img_key

    def _get_image_data(self, data_dir, file_path):
        frame_list = []
        img_key, caption = file_path.split('|||')
        caption = caption.format(*self.placeholder_strings)
        ...
        mask = torch.zeros(self.max_frames, 3, self.resolution[1] // 8, self.resolution[0] // 8)
        if self.use_mask_diffusion:
            mask_key = img_key.replace('jpg', 'png')
            mask_file_path = os.path.join(data_dir, 'masks', mask_key)
            mask_frame_list = []
            for _ in range(5):
                try:
                    ...
                    if image.mode != 'RGB':
                        image = image.convert('RGB')
                    mask_frame_list.append(image) 
                    break
                except Exception as e:
                    logging.info('{} read video frame failed with error: {}'.format(mask_key, e))
                    continue
                
            if len(mask_frame_list) > 0:
                mask_frame_tensor = self.mask_transforms(mask_frame_list)

                # check mask only contains 0 and 1
                unique_values = torch.unique(mask_frame_tensor)
                assert len(unique_values) <= 2 and all(value in [0, 1] for value in unique_values)

                mask[:len(mask_frame_list), ...] = mask_frame_tensor
        
        return ref_frame, vit_frame, video_data, mask, caption
