...
def inference_videolcm_vcomposer_entrance(cfg_update,  **kwargs):
        ...
        with amp.autocast(enabled=True):
            ...
            for partial_keys_one in partial_keys:
                ...
                for i, t in enumerate(timesteps):
                    ...
                    noise_pred_text =  model(latent_model_input, t.repeat(batch_size).to(device=latents.device, dtype=latents.dtype), t_w=None, **model_kwargs_one[0])

                    # perform guidance
                    if do_classifier_free_guidance:
                        # noise_pred_uncond, noise_pred_text = noise_pred.chunk(2)
                        noise_pred_uncond = model(latent_model_input, t.repeat(batch_size).to(device=latents.device, dtype=latents.dtype), t_w=None, **model_kwargs_one[1])
