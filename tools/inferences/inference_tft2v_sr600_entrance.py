...
def inference_tft2v_sr600_entrance(cfg_update,  **kwargs):
...
def worker(gpu, cfg, cfg_update):
    ...
    if hasattr(cfg, "partial_keys"):
        test_list_new = []
        for idx, name_one in enumerate(test_list):
            name_one = name_one.split('|||')[-1]
            for partial_keys_one in cfg.partial_keys:
                name = 'condition'
                for ii in partial_keys_one:
                    name = name + "_" + ii
                cap_name = re.sub(r'[^\w\s]', '', name_one).replace(' ', '_')
                ...
                test_list_new.append(name_one + '|||' + low_video_local_path)
    ...
    for idx, caption in enumerate(test_list):
        if '|||' in caption:
            caption, low_video_local_path = caption.split('|||')
            if '|' in caption:
        ...
        cap_name = re.sub(r'[^\w\s]', '', caption).replace(' ', '_')
        # file_name = low_video_local_path.split('/')[-1].split('.mp4')[0] + '_sr.mp4'
        ...
        local_path = low_video_local_path.split('.mp4')[0] + '_sr.mp4'
